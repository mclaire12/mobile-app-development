...
public class StudentDAO {
    ...
    public List<Student> getAllStudents() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                student.setName(
                        cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_NAME)));
                student.setStudentId(cursor
                        .getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_STUDENT_ID)));
                student.setEnrollmentDate(cursor
                        .getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_ENROLLMENT_DATE)));
                student.setActive(cursor
                        .getInt(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_IS_ACTIVE)) == 1);
                student.setDepartment(cursor
                        .getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_DEPARTMENT)));
                student.setPhotoPath(cursor
                        .getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_PHOTO_PATH)));
    ...
    public Student getStudentById(long id) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            student = new Student();
            student.setId(cursor.getLong(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry._ID)));
            student.setName(cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_NAME)));
            student.setStudentId(
                    cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_STUDENT_ID)));
            student.setEnrollmentDate(cursor
                    .getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_ENROLLMENT_DATE)));
            student.setActive(
                    cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_IS_ACTIVE)) == 1);
            student.setDepartment(
                    cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_DEPARTMENT)));
            student.setPhotoPath(
                    cursor.getString(cursor.getColumnIndexOrThrow(DatabaseContract.StudentEntry.COLUMN_PHOTO_PATH)));
            cursor.close();
        }
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                course.setStudentIdFk(cursor
                        .getLong(cursor.getColumnIndexOrThrow(DatabaseContract.CourseEntry.COLUMN_STUDENT_ID_FK)));
                course.setCourseName(cursor
                        .getString(cursor.getColumnIndexOrThrow(DatabaseContract.CourseEntry.COLUMN_COURSE_NAME)));
                course.setGrade(cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseContract.CourseEntry.COLUMN_GRADE)));
                course.setCourseDate(cursor
                        .getString(cursor.getColumnIndexOrThrow(DatabaseContract.CourseEntry.COLUMN_COURSE_DATE)));
                courses.add(course);
            } while (cursor.moveToNext());
            cursor.close();
        }
